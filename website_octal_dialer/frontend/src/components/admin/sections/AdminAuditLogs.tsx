...
export const AdminAuditLogs: React.FC<AdminAuditLogsProps> = ({
  ...
  const getActionBadge = (action: string) => {
    ...
    return isLight ? 'bg-slate-100 text-slate-700 border-slate-300' : 'bg-[#18181b] text-slate-400 border-[#18181b]';
  };
  ...
  return (
    <div className="space-y-6 text-left">
          ...
          <table className="w-full text-left border-collapse text-xs font-mono">
            ...
            <tbody className={`divide-y ${isLight ? 'divide-slate-200 text-slate-800' : 'divide-[#18181b] text-slate-300'}`}>
              {logs.map((log) => {
                ...
                return (
                  <tr key={log.id} className={isLight ? 'hover:bg-slate-50' : 'hover:bg-[#121215]'}>
                    ...
                    <td className="p-3 text-right">
                      <button
                        ...
                        className={`p-1.5 rounded-lg border transition cursor-pointer ${
                          isLight ? 'border-slate-200 text-slate-500 hover:text-amber-600 hover:bg-slate-100' : 'border-[#18181b] text-slate-400 hover:text-amber-400 hover:bg-[#27272a]'
                        }`}
      ...
      {/* Pagination Footer */}
      {totalPages > 1 && (
        <div className={`flex items-center justify-between text-xs font-mono pt-2 ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
          <span>Showing page {page} of {totalPages} ({totalCount} total entries)</span>
          <div className="flex items-center gap-1.5">
            <button
              disabled={page === 1 || loading}
              onClick={() => setPage(p => Math.max(1, p - 1))}
              className={`px-3 py-1 rounded-lg border disabled:opacity-40 cursor-pointer flex items-center gap-1 ${
                isLight ? 'border-slate-300 bg-white hover:bg-slate-100 text-slate-700' : 'border-[#18181b] hover:bg-[#27272a]'
              }`}
            >
              <ChevronLeft className="w-3 h-3" />
              <span>Prev</span>
            </button>
            <span className={`px-2 font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>{page}</span>
            <button
          ...
          <div className={`w-full max-w-lg border rounded-2xl p-6 shadow-2xl space-y-4 ${
            ...
            <div className="space-y-3 font-mono text-xs max-h-[60vh] overflow-y-auto pr-1">
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div className={`p-2.5 rounded-xl border ${isLight ? 'border-slate-200 bg-slate-50' : 'border-[#18181b] bg-[#121215]'}`}>
                  <span className="text-[9px] text-slate-500 block uppercase">Event ID</span>
                  <span className={`font-bold ${isLight ? 'text-slate-900' : 'text-slate-200'}`}>{selectedLog.id}</span>
                </div>
                <div className={`p-2.5 rounded-xl border ${isLight ? 'border-slate-200 bg-slate-50' : 'border-[#18181b] bg-[#121215]'}`}>
                  <span className="text-[9px] text-slate-500 block uppercase">Actor</span>
                  <span className="font-bold text-amber-500">{selectedLog.username}</span>
                </div>
                <div className={`p-2.5 rounded-xl border ${isLight ? 'border-slate-200 bg-slate-50' : 'border-[#18181b] bg-[#121215]'}`}>
                  <span className="text-[9px] text-slate-500 block uppercase">Action Type</span>
                  <span className={`font-bold ${isLight ? 'text-slate-900' : 'text-slate-200'}`}>{selectedLog.action}</span>
                </div>
                <div className={`p-2.5 rounded-xl border ${isLight ? 'border-slate-200 bg-slate-50' : 'border-[#18181b] bg-[#121215]'}`}>
                  <span className="text-[9px] text-slate-500 block uppercase">IP Address</span>
                  <span className={`font-bold ${isLight ? 'text-slate-900' : 'text-slate-200'}`}>{selectedLog.ipAddress || '127.0.0.1'}</span>
                </div>
              </div>

              {selectedLog.userAgent && (
                <div className={`p-2.5 rounded-xl border text-[10px] ${isLight ? 'border-slate-200 bg-slate-50' : 'border-[#18181b] bg-[#121215]'}`}>
                  <span className="text-[9px] text-slate-500 block uppercase">Client User Agent</span>
                  <span className="text-slate-500 break-all">{selectedLog.userAgent}</span>
                </div>
              )}

              <div>
                <span className={`text-[10px] uppercase font-bold block mb-1 ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>Structured Payload (Sanitized)</span>
                <pre className={`p-3 rounded-xl border text-[11px] overflow-x-auto whitespace-pre-wrap ${
                  isLight ? 'bg-slate-50 border-slate-200 text-amber-700 font-mono' : 'border-[#18181b] bg-[#18181b] text-amber-300'
                }`}>
            ...
            <div className={`flex justify-end pt-2 border-t ${isLight ? 'border-slate-200' : 'border-[#18181b]'}`}>
              <button
                onClick={() => setSelectedLog(null)}
                className={`px-4 py-2 font-mono text-xs rounded-xl transition cursor-pointer border ${
